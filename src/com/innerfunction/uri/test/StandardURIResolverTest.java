...
public class StandardURIResolverTest extends AndroidTestCase {
    ...
    public void testLocal() throws URISyntaxException{
        ...
        edit.commit();
        assertEquals(prefs.getString("email", ""),sur.dereference("local:email").toString());
        assertEquals(prefs.getString("name", ""),sur.dereference("local:name").toString());
    }
    ...
    public void testCache() throws IOException, URISyntaxException{
        ...
        assertEquals("cache:%2Ftemp.tmp",sur.dereference("cache:/temp.tmp").getURI().toString());
        assertEquals("cache", sur.dereference("cache:/temp.tmp").getURI().getScheme());
        assertEquals("This is the temporary file content", sur.dereference("cache:/temp.tmp").toString());
        assertEquals("/storage/emulated/0/Android/data/com.innerfunction.semo/files/Download/temp.tmp", sur.dereference("cache:/temp.tmp").asDefault().toString());
        outputFile.delete();
    }
    
    public void testApp() throws IOException{
        assertNotNull(sur.dereference("app:/configuration.json"));
        assertEquals("configuration.json", sur.dereference("app:/configuration.json").asDefault().toString());
        assertEquals(FileIO.readString(getContext().getAssets().open("configuration.json"),"json" ,"UTF-8"),sur.dereference("app:/configuration.json").toString());
    }
    
    public void tests(){
        assertEquals("name",sur.dereference(uri).toString());
        assertEquals("s",sur.dereference(uri).getURI().getScheme());
    }
}
